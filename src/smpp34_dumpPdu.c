...
#define U32( inst, par, _str ){\
    ...
    lenval = snprintf(l_dest, sizeof(l_dest), "%-30s[%08X] - [%s]\n",\
...
#define U16( inst, par, _str ){\
    ...
    lenval = snprintf(l_dest, sizeof(l_dest),\
                               "%-30s[%04X] [%s]\n",#par,inst par,dummy_b);\
    if( lenval >= left ){\
...
#define U08( inst, par, _str ){\
    ...
    lenval = snprintf(l_dest, sizeof(l_dest), "%-30s[%02X]       - [%s]\n",\
                                                      #par,inst par, dummy_b);\
    if( lenval >= left ){\
...
#define _op(inst, par, size ){\
    lenval = snprintf(l_dest, sizeof(l_dest), "%-30s[%."#size"s]\n",\
                                                           #par, inst par);\
    if( lenval >= left ){\
...
#define OCTET8( inst, par, size ){\
    ...
    p += snprintf(p, (sizeof(l_dest)-(p-l_dest)), "]\n");\
    dummy = snprintf(dest, left, "%s", l_dest);\
